...
from tally_migration.migration import record_guard as rg
...
class TestGuardConfig(unittest.TestCase):
    ...
    def test_timeout_override(self):
        with self._conf(tally_migration_record_timeout=30):
            self.assertEqual(rg.record_timeout_s(), 30)

    def test_timeout_invalid_falls_back(self):
        with self._conf(tally_migration_record_timeout=-5):
            self.assertEqual(rg.record_timeout_s(), 60)
        with self._conf(tally_migration_record_timeout="nonsense"):
            self.assertEqual(rg.record_timeout_s(), 60)

    def test_max_resumes_default_and_override(self):
        with self._conf():
            self.assertEqual(rg.max_resumes(), 50)
        with self._conf(tally_migration_max_resumes=10):
...
class TestActualKill(unittest.TestCase):
    ...
    def test_hung_record_is_dumped_and_killed(self):
        ...
        dump = os.path.join(dump_dir, "tally_migration_hangs", "LKILL.log")
...
from tally_migration.erpnext.importers.base import BaseImporter
...
class TestResumeSweep(unittest.TestCase):
    ...
    def test_dead_run_with_marker_reenqueues_and_records_stall(self):
        from tally_migration.migration import resume
        log = self._log()
