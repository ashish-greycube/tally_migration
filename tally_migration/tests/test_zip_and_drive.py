...
from tally_migration.tally.file_source import unzip_if_zip
...
class TestConfiguredMaxUpload(unittest.TestCase):
    ...
    ``bench set-config tally_migration_max_upload_mb 1024`` stores the STRING
    ...
    def _with_conf(self, value):
        conf = {} if value is _UNSET else {"tally_migration_max_upload_mb": value}
...
class TestDownloadDriveFile(unittest.TestCase):
    def _patch_conf(self):
        ...
        return mock.patch.object(api.frappe, "conf", {"tally_migration_max_upload_mb": 10})
