...
class TestTallyExtractor(unittest.TestCase):
    ...
    def test_source_company_is_not_imported_as_customer(self):
        class CompanyClient(_MockClient):
            def source_company(self):
                return "  customer a  "

        masters = TallyExtractor(CompanyClient()).extract_all()
        self.assertEqual([c["_name"] for c in masters.customers], ["Customer B"])
        self.assertEqual([c["_name"] for c in masters.excluded_parties], ["Customer A"])
        self.assertEqual(masters.source_company, "  customer a  ")
