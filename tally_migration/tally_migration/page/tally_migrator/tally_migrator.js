...
class TallyMigratorPage {
		...
		$(this.wrapper).find(".page-content").html(`
			...
			<div class="container tally-migrator">
				...
				<div id="section-upload">
					...
					<p id="upload-intro" class="text-muted tm-lead">
					...
					<div class="tm-section" style="margin-top:var(--margin-lg);">
						<div class="tm-label">What do you want to import?</div>
						<div class="tm-types">
							<button class="btn btn-default tm-import-type" data-import-type="masters">
								<span class="tm-type-title">Masters</span>
								<span class="tm-type-sub">Accounts, customers, suppliers, items and opening balances</span>
							</button>
							<button class="btn btn-default tm-import-type" data-import-type="daybook">
								<span class="tm-type-title">Day Book</span>
								<span class="tm-type-sub">Accounting vouchers, imported as Journal Entries</span>
							</button>
						</div>
					</div>

					<div id="masters-export-guidance" class="tm-guide tm-section">
						<div class="tm-guide-head"><span class="tm-badge">1</span> Export from Tally</div>
						<ol class="tm-steps">
							<li>Open your company in <strong>Tally Prime</strong>.</li>
							<li>Go to <strong>Gateway of Tally → Export → Masters</strong>, then open <strong>Configure</strong>.</li>
							<li>Set these options:
								<div class="tm-opts">
									<div class="tm-opt"><span>Type of master</span><b class="tm-val">All Masters</b></div>
									<div class="tm-opt"><span>Include dependent masters</span><b class="tm-val">Yes</b></div>
									<div class="tm-opt"><span>Export closing balance as opening balance</span><b class="tm-val">Yes</b></div>
									<div class="tm-opt"><span>File format</span><b class="tm-val">XML (Data Interchange)</b></div>
								</div>
								<div class="text-muted small" style="margin-top:var(--margin-xs);">The closing balance becomes your <span class="tm-hl">opening balances</span> in ERPNext.</div>
							</li>
							<li>Choose <strong>Export</strong> and note where <code>Master.xml</code> is saved.</li>
						</ol>
					</div>

					<div id="daybook-export-guidance" class="tm-guide tm-section" style="display:none;">
						<div class="tm-guide-head"><span class="tm-badge">1</span> Export your Day Book from Tally</div>
						<ol class="tm-steps">
							<li>Open the company in <strong>Tally Prime</strong>.</li>
							<li>Go to <strong>Display More Reports → Day Book</strong>, then export it.</li>
							<li>Choose file format <b class="tm-val">XML (Data Interchange)</b>.</li>
						</ol>
						${TallyMigratorPage.callout("info", TallyMigratorPage.iconRow("info", `<strong>Import Masters first.</strong> Day Book vouchers post to accounts, customers and suppliers that must already exist. Keep the Day Book <span class="tm-hl">starting the day after</span> your Masters opening-balance date. Vouchers dated before it are already in your opening balances, so they are <span class="tm-hl">skipped automatically</span>.`))}
						<p class="text-muted small" style="margin:var(--margin-sm) 0 0;">Accounting-only vouchers import as <span class="tm-hl">Journal Entries</span>. Vouchers with stock items, cancelled vouchers and Tally's system <span class="tm-hl">Profit &amp; Loss A/c</span> entries are skipped and listed in the log.</p>
					</div>

					<div class="tm-guide tm-section">
						<div class="tm-guide-head"><span class="tm-badge">2</span> Bring it in here</div>
						<div class="tm-drop" id="tm-drop" role="button" tabindex="0">
							<button id="btn-pick-file" class="btn btn-default btn-sm">
								${TallyMigratorPage.navIcon("upload")} &nbsp;Choose Tally XML or .zip file
							</button>
							<span id="file-status" class="text-muted"></span>
						</div>
						<p class="text-muted small" style="margin:var(--margin-sm) 0 0;">
				...
				<div id="section-configure" style="display:none;">
					<h4 id="configure-title">Choose where the data should go</h4>
					<p id="configure-intro" class="text-muted">Select the ERPNext company that will receive these records.</p>

					<!-- Company is required for both Masters and Day Book imports, so this row is
					     never hidden - only masters-only-options (below) toggles with import type. -->
					<div class="row">
					...
					<div id="masters-only-options" class="row">
						<div class="form-group col-sm-6">
	...
	isDayBook() {
		return this.importType === "daybook";
	}

	setImportType(type) {
		if (this.fileUrl) {
			frappe.msgprint(__("Start over before changing the import type."));
			return;
		}
		this._applyImportType(type);
	}

	// The actual state + UI change, without the "already has a file" guard - shared
	// by the user-facing toggle (setImportType) and draft resume, which must restore
	// the saved type even though fileUrl is already set by then.
	_applyImportType(type) {
		this.importType = type;
		const isDayBook = this.isDayBook();
		$(".tm-import-type")
			.removeClass("btn-primary")
			.addClass("btn-default");
		$(`.tm-import-type[data-import-type="${type}"]`)
			.removeClass("btn-default")
			.addClass("btn-primary");
		$("#masters-export-guidance").toggle(!isDayBook);
		$("#daybook-export-guidance").toggle(isDayBook);
		$("#upload-intro").text(
			isDayBook
				? __("This tool imports accounting-only Tally Day Book vouchers as Journal Entries. It checks the file first, so nothing changes until you are ready.")
				: __("This tool brings your Tally masters, accounts and opening balances into ERPNext. It checks your file and shows you a preview first, so nothing changes until you are ready.")
		);
		this.renderStepper(this._currentStep || "section-upload");
	}
	...
	bindEvents() {
		...
		$(".tm-import-type").on("click", (e) => this.setImportType($(e.currentTarget).data("import-type")));
		this.setImportType(this.importType);
		// The whole drop zone is the click target (the button inside bubbles to it).
		$("#tm-drop").on("click", () => this.pickFile()).on("keydown", (e) => {
			if (e.key === "Enter" || e.key === " ") { e.preventDefault(); this.pickFile(); }
		});
	...
	dayBookSummaryHtml(summary) {
		const T = TallyMigratorPage;
		const types = Object.entries(summary.by_type || {})
			.map(([type, count]) => `<span class="tm-chip"><strong>${count}</strong> ${frappe.utils.escape_html(type)}</span>`)
			.join("");
		const company = summary.source_company
			? `<div style="margin-bottom:var(--margin-sm);">${__("Tally Company")}: <strong>${frappe.utils.escape_html(summary.source_company)}</strong></div>`
			: "";
		const skipped = (summary.inventory || 0) + (summary.cancelled || 0) + (summary.unsupported || 0);
		const cards = `<div class="tm-stats" style="margin:var(--margin-sm) 0;">
			${T.statCard(summary.supported || 0, __("Ready to import"), __("as Journal Entries"), "green")}
			${T.statCard(skipped, __("Will be skipped"), skipped ? __("listed in the log") : __("none"), skipped ? "blue" : "gray",
				__("Inventory, cancelled and unsupported vouchers are not imported. Each one is listed in the migration log."))}
			${T.statCard(summary.total || 0, __("Total vouchers"), __("in this file"), "gray")}
		</div>`;
		return `<div style="margin-top:var(--margin-sm);">${company}${cards}<div class="tm-chips">${types}</div></div>`;
	}
	...
	loadDayBookPreview() {
		frappe.call({
			method: "tally_migration.api.preview_daybook_file",
			args: { file_url: this.fileUrl },
			callback: (r) => {
				this.preview = r.message || {};
				this.renderStepper("section-upload");
				if (!this.preview.supported) {
					$("#preview-box").html(
						TallyMigratorPage.callout("error", TallyMigratorPage.iconRow(
							"error", __("No supported accounting vouchers were found in this Day Book file.")
						))
					);
					return;
				}
				$("#preview-box").html(
					TallyMigratorPage.callout("success", TallyMigratorPage.iconRow(
						"success", `<strong>${__("Day Book file read successfully.")}</strong>${this.dayBookSummaryHtml(this.preview)}`
					))
				);
				$("#btn-next-upload").prop("disabled", false);
			},
			error: () => this._onPreviewFailed(),
		});
	}
	...
	_pollPreview(gen) {
		...
		frappe.call({
			method: "tally_migration.api.preview_masters_file",
	...
	_onPreviewFailed(reason) {
		...
		const fileType = this.isDayBook() ? "Day Book" : "Masters";
		$("#preview-box").html(
			TallyMigratorPage.callout("error", TallyMigratorPage.iconRow("error", `We couldn't read this file.${detail} Please make sure it's a valid Tally <strong>${fileType} XML</strong> export and upload it again.`))
		);
		$("#btn-next-upload").prop("disabled", true);
	}

	countsHtml(p) {
		if (this.isDayBook()) return this.dayBookSummaryHtml(p);
		const accounts = (p.account_groups || 0) + (p.ledger_accounts || 0);
		const rows = [
		...
		const big = rows
			.filter(([, , show]) => show)
			.slice(0, 3)
			.map(([label, n]) => T.statCard(n, label, n ? __("ready to import") : __("none"), n ? "green" : "gray"))
			.join("");
		const chips = rows
			.filter(([, , show]) => show)
			.slice(3)
			...
			.join("");
		const source = p.source_company
			? `<div class="text-muted" style="margin-top:8px;">${__("Tally Company")}: <strong>${frappe.utils.escape_html(p.source_company)}</strong></div>`
			: "";
		const excluded = (p.excluded_party_names || []).length
			? `<div class="text-muted" style="margin-top:4px;">${__("Not imported as a party because it matches the Tally company")}: <strong>${frappe.utils.escape_html(p.excluded_party_names.join(", "))}</strong></div>`
			: "";
		return `<div class="tm-stats" style="margin:var(--margin-sm) 0;">${big}</div><div class="tm-chips">${chips}</div>${source}${excluded}`;
	}

	proceedToConfigure() {
		this.loadERPNextCompanies();
		const isDayBook = this.isDayBook();
		$("#configure-title").text(isDayBook ? __("Choose the company for this Day Book") : __("Choose where the data should go"));
		$("#configure-intro").text(
			isDayBook
				? __("Select the existing ERPNext company where Journal Entries should be created.")
				: __("Select the ERPNext company that will receive these records.")
		);
		$("#masters-only-options").toggle(!isDayBook);
	...
	saveDraft() {
		...
		this._draftTimer = setTimeout(() => {
			...
			frappe.call({
				method: "tally_migration.api.save_draft",
	...
	flushDraft() {
		...
		try {
			...
			navigator.sendBeacon("/api/method/tally_migration.api.save_draft", fd);
			this._draftPending = null;
		} catch (e) {
	...
	loadDraft() {
		frappe.call({
			method: "tally_migration.api.get_draft",
			callback: (r) => {
				const d = r.message;
	...
	resumeDraft(d) {
		// Must happen before fileUrl is set (below) and before any preview/check/run
		// logic runs - every one of those branches on isDayBook(), and a draft saved
		// mid Day Book import must not resume as if it were a Masters import.
		this._applyImportType(d.import_type || "masters");
	...
	reconnectOnResume(d) {
		frappe.call({
			method: "tally_migration.api.active_run",
	...
	clearDraft() {
		...
		$("#resume-banner").hide().empty();
		frappe.call({
			method: "tally_migration.api.clear_draft",
			callback: () => {},
			error: () => {
	...
	loadERPNextCompanies() {
		frappe.call({
			method: "tally_migration.api.get_companies",
			callback: (r) => {
	...
	proceedToCheck() {
		if (this.isDayBook()) {
			this.proceedDayBookToCheck();
			return;
		}
		$("#section-check h4").text(__("Quick check before we begin"));
		$("#section-check > p").text(
			__("We compare the data in your file against what already exists in ERPNext, so you can decide how to handle anything that doesn't match - nothing is changed automatically.")
		);
	...
	proceedDayBookToCheck() {
		const summary = this.preview || {};
		$("#section-check h4").text(__("Review your Day Book import"));
		$("#section-check > p").text(
			__("Only balanced, accounting-only vouchers are imported as Journal Entries. Inventory and cancelled vouchers are skipped and listed in the migration log.")
		);
		$("#check-loading, #check-failed, #dq-section, #readiness-section, #coverage-section, #check-issues, #dq-consent").hide();
		$("#check-clean")
			.html(TallyMigratorPage.callout("info", TallyMigratorPage.iconRow(
				"info", `<strong>${summary.supported || 0}</strong> ${__("accounting vouchers are ready to import.")}${this.dayBookSummaryHtml(summary)}`
			)))
			.show();
		this._dayBookBlocked = true;
		$("#check-clean").append('<div id="daybook-date-check" class="text-muted" style="margin-top:var(--margin-md);"><span class="tm-spin"></span> &nbsp;Checking dates against your Masters opening balance...</div>');
		this.show("section-check");
		this._updateCheckContinue();
		frappe.call({
			method: "tally_migration.api.daybook_date_check",
			args: { file_url: this.fileUrl, erpnext_company: this.getCompany() },
			callback: (r) => this._renderDayBookDateCheck(r.message || {}),
			// The date check is a safety net: if it can't run, say so and require consent.
			error: () => this._renderDayBookDateCheck({ failed: true }),
		});
	}

	_renderDayBookDateCheck(d) {
		const T = TallyMigratorPage;
		const esc = frappe.utils.escape_html;
		const fmt = (x) => (x ? frappe.datetime.str_to_user(x) : "-");
		let html = "";
		if (d.failed) {
			html = T.callout("info", T.iconRow("info", `<strong>We couldn't check the voucher dates.</strong> Vouchers dated before your Masters opening-balance date are skipped automatically, as they are already in the opening balances. You can still continue.`));
		} else if (!d.opening_date) {
			html = T.callout("info", T.iconRow("info", `<strong>No Masters import found for this company.</strong> Day Book vouchers need the accounts, customers and suppliers from a Masters import. Import <span class="tm-hl">Masters first</span>, otherwise most vouchers will fail.`));
		} else if (d.before_count > 0) {
			html = T.callout("info", T.iconRow("info", `<strong>${d.before_count} of ${d.total} vouchers are already in your opening balance.</strong>
				<div style="margin-top:var(--margin-xs);">Opening balances date: <span class="tm-hl">${esc(fmt(d.opening_date))}</span> &nbsp;·&nbsp; Vouchers run from <span class="tm-hl">${esc(fmt(d.first_voucher))}</span> to ${esc(fmt(d.last_voucher))}.</div>
				<div style="margin-top:var(--margin-xs);">Your opening balances already include everything up to that date, so these vouchers will be <span class="tm-hl">skipped automatically</span> and not imported twice. Just continue.</div>`));
		} else {
			html = T.callout("success", T.iconRow("success", `<strong>Dates look good.</strong> All ${d.total} vouchers fall on or after your opening balance date (${esc(fmt(d.opening_date))}).`));
		}
		$("#daybook-date-check").replaceWith(`<div id="daybook-date-check" style="margin-top:var(--margin-md);">${html}</div>`);
		this._dayBookBlocked = false;
		this._updateCheckContinue();
	}
	...
	_pollPreflight() {
		frappe.call({
			method: "tally_migration.api.validate_masters_data",
	...
	_updateCheckContinue() {
		...
		const blocked = loading || !!this.qualityError || blockers || (hasErrors && !consented) ||
			(this.isDayBook() && (!((this.preview || {}).supported) || !!this._dayBookBlocked));
	...
	recheckReadiness() {
		...
		frappe.call({
			method: "tally_migration.api.company_readiness",
	...
	static themeStyle() {
		return `<style>
			[data-theme="dark"] .tally-migrator {
				...
				--red-100: #361515;    --red-200: #521515;   --yellow-100: #3a3000;  --card-bg: #1c1c1c;  --gray-400: #555;
				--gray-100: #232323;   --gray-200: #2b2b2b;   --gray-300: #343434;
			}


			/* ── Import type cards, guides, drop zone ─────────────────────────── */
			.tally-migrator .tm-label { font-weight: 600; margin-bottom: var(--margin-sm); }
			.tally-migrator .tm-types { display: grid; grid-template-columns: 1fr 1fr; gap: var(--margin-md); }
			.tally-migrator .tm-import-type {
				text-align: left; white-space: normal; padding: var(--padding-md);
				border: 1px solid var(--gray-400, #c0c6cc); border-radius: var(--border-radius-md, 8px);
				display: flex; flex-direction: column; gap: 2px; height: auto;
			}
			.tally-migrator .tm-import-type.btn-primary { border-color: var(--text-color); box-shadow: 0 0 0 1px var(--text-color); }
			.tally-migrator .tm-type-title { font-size: var(--text-lg); font-weight: 600; }
			.tally-migrator .tm-type-sub { font-size: var(--text-sm); opacity: .75; }
			.tally-migrator .tm-guide {
				border: 1px solid var(--gray-400, #c0c6cc);
				border-radius: var(--border-radius-md, 8px); background: var(--card-bg, #fff);
				padding: var(--padding-md) var(--padding-lg); margin-top: var(--margin-lg);
			}
			.tally-migrator .tm-guide-head { font-weight: 600; font-size: var(--text-lg); margin-bottom: var(--margin-sm); display: flex; align-items: center; gap: var(--margin-sm); }
			.tally-migrator .tm-badge {
				display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px;
				border-radius: 50%; background: var(--text-color); color: var(--bg-color); font-size: var(--text-sm);
			}
			.tally-migrator .tm-steps { margin: 0; padding-left: 20px; line-height: 1.8; }
			.tally-migrator .tm-opts { margin-top: var(--margin-xs); border: 1px solid var(--gray-300, #d1d8dd); border-radius: var(--border-radius); overflow: hidden; }
			.tally-migrator .tm-opt { display: flex; justify-content: space-between; gap: var(--margin-md); padding: 6px 12px; line-height: 1.5; }
			.tally-migrator .tm-opt + .tm-opt { border-top: 1px solid var(--gray-300, #d1d8dd); }
			.tally-migrator .tm-opt:nth-child(even) { background: var(--gray-100, #f4f5f6); }
			.tally-migrator .tm-val { background: var(--yellow-100, #fff8e1); border-bottom: 2px solid var(--yellow-500, #f5b301); padding: 0 6px; border-radius: 2px; white-space: nowrap; }
			.tally-migrator .tm-opt .tm-val { background: none; border-bottom: 0; padding: 0; }
			.tally-migrator .tm-drop { cursor: pointer; transition: background .15s, border-color .15s; }
			.tally-migrator .tm-drop:hover, .tally-migrator .tm-drop:focus { border-color: var(--text-color); background: var(--gray-200, #ebeef0); outline: 0; }
			.tally-migrator .tm-drop {
				display: flex; align-items: center; gap: var(--margin-md); padding: var(--padding-lg);
				border: 2px dashed var(--gray-400, #c0c6cc); border-radius: var(--border-radius-md, 8px); background: var(--gray-100, #f4f5f6);
			}
			...
			.tally-migrator h4 { font-size: var(--text-xl); font-weight: 700; margin: 0 0 var(--margin-sm); }
			.tally-migrator .tm-stepper { padding-bottom: var(--padding-md); border-bottom: 1px solid var(--border-color); }
			.tally-migrator h5 { font-size: var(--text-md); font-weight: 600; margin: 0 0 var(--margin-sm); }
			.tally-migrator p { line-height: 1.6; }
			.tally-migrator .tm-hl { background: var(--yellow-100, #fff8e1); border-bottom: 2px solid var(--yellow-500, #f5b301); padding: 0 3px; border-radius: 2px; font-weight: 600; }
			...
			.tally-migrator .tm-footer {
				margin-top: var(--margin-xl); padding-top: var(--padding-md); border-top: 1px solid var(--border-color); display: flex;
			...
			.tally-migrator .tm-stat {
				flex: 1; border: 1px solid var(--gray-400, #c0c6cc); background: var(--card-bg, #fff);
			...
			.tally-migrator .tm-chip {
				display: inline-block; margin: var(--margin-xs) var(--margin-sm) 0 0; border: 1px solid var(--gray-300, #d1d8dd);
	...
	resolveUomsAndContinue() {
		...
		frappe.call({
			method: "tally_migration.api.create_uoms",
	...
	_setupProgressStream() {
		if (!this._onProgress) {
			this._onProgress = (data) => {
				const expectedTitle = this.isDayBook() ? "Tally Day Book Migration" : "Tally Masters Migration";
				if (data.title !== expectedTitle) return;
	...
	runMigration() {
		...
		frappe.call({
			...
			callback: (r) => {
				...
				if (this.isDayBook()) {
					this._startDayBookMigration(erpnext);
				} else {
	...
	_startDayBookMigration(erpnext) {
		this._setupProgressStream();
		frappe.call({
			method: "tally_migration.api.run_daybook_migration_from_file",
			args: { file_url: this.fileUrl, erpnext_company: erpnext },
			callback: (r) => {
				const result = r.message || {};
				if (result.enqueued && result.log_name) {
					$("#progress-desc").text(__("Day Book import is running in the background. You can leave this page; the result will appear here and in the migration log."));
					this.pollLog(result.log_name);
					return;
				}
				this._showMigrationStartError();
			},
			error: () => this._showMigrationStartError(),
		});
	}

	_showMigrationStartError() {
		frappe.realtime.off("tally_migration_progress", this._onProgress);
		this.stopHeartbeat();
		$("#progress-section").hide();
		$("#btn-run, #btn-back-3").prop("disabled", false);
		$("#error-section")
			.html(`<strong>${__("Migration could not start.")}</strong> ${__("See the error dialog for details, then try again.")}`)
			.show();
	}
	...
	_startMigration(erpnext, overrides) {
		...
		frappe.call({
			method: "tally_migration.api.run_masters_migration_from_file",
			args: {
				file_url: this.fileUrl,
	...
	pollLog(logName) {
		...
		const reportRunStatus = () => {
			...
			frappe.call({
				method: "tally_migration.api.run_liveness",
		...
		const poll = () => {
			...
			frappe.call({
				method: "tally_migration.api.run_progress",
				args: { log_name: logName },
				callback: (r) => {
					const doc = r.message;
	...
	renderResults(summary) {
		...
		const totSkipped = entries.reduce((a, [, r]) => a + (r.skipped || 0), 0);
		const totFailed = entries.reduce((a, [, r]) => a + (r.failed || 0), 0);
		const T = TallyMigratorPage;
		html += `<div class="tm-stats" style="margin-top:var(--margin-md);">
			${T.statCard(totalCreated, __("Imported"), __("new records"), totalCreated ? "green" : "gray")}
			${T.statCard(totSkipped, __("Already there / skipped"), __("nothing changed"), "gray")}
			${T.statCard(totalWarnings, __("Warnings"), totalWarnings ? __("please review") : __("none"), totalWarnings ? "blue" : "gray")}
			${T.statCard(totFailed, __("Failed"), totFailed ? __("see the log") : __("none"), totFailed ? "red" : "gray")}
		</div>`;
