...
function rerun(frm) {
	...
	frappe.call({
		method: "tally_migration.api.rerun_from_log",
		args: { log_name: frm.doc.name },
		callback: (r) => {
			frappe.dom.unfreeze();
			const res = r.message || {};
...
function tally_rollback_confirm(frm, total) {
	...
	frappe.call({
		method: "tally_migration.migration.rollback.preview_revert",
		args: { log_name: frm.doc.name },
...
function tally_rollback_run(frm, company_confirmation) {
	...
	frappe.call({
		method: "tally_migration.migration.rollback.revert_migration",
