...
from .mappings import TALLY_ROOT_PARENT, classify_group, is_system_ledger, norm_group
...
class ExtractedMasters:
    ...
    source_company: str = ""
    excluded_parties: list[dict] = field(default_factory=list)
    ...
    def summary(self) -> dict:
        return {
            ...
            "excluded_parties": len(self.excluded_parties),
        }
...
class TallyExtractor:
    ...
    def extract_all(self) -> ExtractedMasters:
        ...
        source_company = self.client.source_company() if hasattr(self.client, "source_company") else ""
        company_key = norm_group(source_company)
        excluded_parties = [
            l for l in ledgers
            if company_key
            and norm_group(l["_name"]) == company_key
            and resolver.kind_of(l["_name"]) in (CUSTOMER, SUPPLIER)
        ]
        excluded_names = {l["_name"] for l in excluded_parties}
        masters = ExtractedMasters(
            customers    = [
                l for l in ledgers
                if resolver.kind_of(l["_name"]) == CUSTOMER and l["_name"] not in excluded_names
            ],
            suppliers    = [
                l for l in ledgers
                if resolver.kind_of(l["_name"]) == SUPPLIER and l["_name"] not in excluded_names
            ],
