...
# these (every other element - TALLYMESSAGE wrappers and voucher data when
# present - is discarded as it is parsed). COMPANY is retained only for the
# source-company name used by the party exclusion check, so peak memory stays
...
MASTER_RECORD_TAGS = (
    ...
    "COSTCENTRE", "STOCKGROUP", "UNIT", "CURRENCY", "COMPANY",
)
...
def unzip_if_zip(raw, max_uncompressed_bytes: int):
    ...
    if member.file_size > max_uncompressed_bytes:
        ...
        frappe.throw(
            ...
            "ask your administrator to raise tally_migration_max_upload_mb."
        )
    try:
...
class FileTallySource:
    ...
    def source_company(self) -> str:
        """Return the company identity stored in the Tally export header."""
        for company in self._by_tag.get("COMPANY", ()):
            for elem in company.iter():
                if elem.tag.split("}")[-1].upper() == "REMOTECMPNAME":
    ...
    def approx_record_count(self) -> int:
        ...
        return sum(len(v) for tag, v in self._by_tag.items() if tag != "COMPANY")
