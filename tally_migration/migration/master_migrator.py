...
from tally_migration.tally.extractors import TallyExtractor, ExtractedMasters
from tally_migration.erpnext.importers import ERPNextImporter, ImportResult
from tally_migration.migration.overrides import apply_record_overrides, uom_edits
...
class MasterMigrator:
    ...
    def run(self) -> MigrationSummary:
        ...
        try:
            ...
            self._record_excluded_parties(results, masters)
    ...
    @staticmethod
    def _record_excluded_parties(results: dict, masters: ExtractedMasters) -> None:
        """Show source-company ledgers that were withheld from party creation."""
        if not masters.excluded_parties:
            return
        result = results.setdefault("Excluded Parties", ImportResult("Customer"))
        for row in masters.excluded_parties:
            result.skipped += 1
            result.add_warning(
                row["_name"],
                "Ledger matches the source Tally company and was not imported as a Customer or Supplier",
            )
    ...
    def _reconciliation(self, masters: ExtractedMasters, coa) -> dict:
        ...
        try:
            from tally_migration.migration.reconciliation import build_reconciliation
