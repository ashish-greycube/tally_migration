...
def record_timeout_s() -> int:
    ...
    high. Override with ``tally_migration_record_timeout`` in site_config."""
    try:
        v = int(frappe.conf.get("tally_migration_record_timeout") or _DEFAULT_TIMEOUT_S)
...
def max_resumes() -> int:
    ...
    ``tally_migration_max_resumes``."""
    try:
        v = int(frappe.conf.get("tally_migration_max_resumes") or _DEFAULT_MAX_RESUMES)
...
def hang_dump_path(log_name: str) -> str:
    ...
    return os.path.join(frappe.get_site_path("tally_migration_hangs"), f"{log_name}.log")
...
class RecordGuard:
    ...
    def _dump_path(self) -> str:
        os.makedirs(frappe.get_site_path("tally_migration_hangs"), exist_ok=True)
